...
- **Account deletion requirement triggered?** No, while NearTime has no account creation.
- **Contains ads?** No.

Do not answer "No data collected" merely because NearTime avoids storing raw location. Google treats data transmitted off-device as collection unless a specific exemption applies.

## Current architecture relevant to Data Safety

- Foreground Android location is optional.
- Current-location search can access approximate or precise location supplied by Android.
- The selected origin is sent over HTTPS to NearTime's Supabase Edge Function.
...
- Custom place/address text is sent through the NearTime backend to TomTom Suggest/Details when the user explicitly uses that flow.
- A random app-install identifier is generated locally. A SHA-256 installation hash is sent to the search backend for anonymous quota/idempotency. The billing path can send the installation identifier to NearTime's billing backend.
...
- When Google Play Billing is used, the app receives a Google Play purchase token and sends it to NearTime's backend for server-side verification. The backend persists pseudonymous/HMAC-derived entitlement or purchase identifiers and product/subscription state; raw payment-card details are not accessed by NearTime.
- The current native Android dependency list does **not** include the Google Maps Android SDK. Google Places calls are server-side and "Open in Maps"/route actions are external intents/URLs. Re-check the final dependency tree before submission.
- The native app does include Google Play Billing Library; review Google's SDK/data guidance for the exact release version.

## Working data-type declaration

| Play data type | Collected? | Shared? | Ephemeral? | Optional / required | Purpose / release note |
| --- | --- | --- | --- | --- | --- |
| Approximate location | Yes, when Current location is used and Android supplies approximate location | Determine under current service-provider/user-initiated transfer rules before submission | Raw origin is intended not to be persisted by NearTime's application database | Optional | App functionality: nearby search and walking results |
| Precise location | Yes, when permission/use supplies precise location | Determine under current service-provider/user-initiated transfer rules before submission | Raw origin is intended not to be persisted by NearTime's application database | Optional | App functionality: more accurate nearby search |
| Other user-generated content: custom place/address text | Yes, only when the user uses Other place | Determine TomTom/service-provider classification before submission | Intended to be request-scoped in NearTime application storage | Optional | App functionality: resolve a user-selected origin |
...
NearTime does **not** access a user's payment-card number or bank credentials. Google Play collects payment credentials directly. Under Google's Data safety guidance, payment data collected directly by the payment service does not need to be declared by the app when the app never accesses it.

NearTime **does** receive purchase/entitlement information, including purchase tokens and verified product/subscription state. Therefore the final form must consider **Purchase history** separately from payment-card information.
...
- raw search-origin coordinates are not intentionally written to the NearTime application database;
- typed custom address/place queries are not intentionally written to the NearTime application database;
...
NearTime currently has no user account. Google's account-deletion requirement therefore does not apply to the current architecture.

The Data safety form separately asks about data-deletion mechanisms. Do not claim a deletion feature that does not exist. If a deletion-request mechanism is later added, document exactly which NearTime-held records it covers and any legitimate retention exceptions.
...
- no unused Google Maps Android SDK/API-key configuration;
...
- NearTime currently serves no ads.
- Search/purchase authorization is pseudonymous and does not require a NearTime name, email address or password.
